...
void main()
{
    ...
    vec3 ambient = u_material.diffuse * 0.01;
    vec3 diffuse = u_directional_light.color * max(dot(normal, light), 0.0) * u_material.diffuse;
    vec3 specular = u_directional_light.color * pow(max(dot(reflection, view), 0.0000001), u_material.shininess) * u_material.specular;
    ...
    specular += attentuation * u_point_light.specular * pow(max(dot(reflection, view), 0.0000001), u_material.shininess) * u_material.specular;
    ...
    if(theta > u_spot_light.outerCone) {
        ...
        specular += intensity * u_spot_light.specular * pow(max(dot(reflection, view), 0.0000001), u_material.shininess) * u_material.specular;
